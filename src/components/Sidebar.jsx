...
const NAV = [
  ...
  { id: "analytics", path: "/analytics", label: "Аналитика", icon: (
      <svg className="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
        <path d="M4 20v-7" />
        <path d="M9 20v-4" />
        <path d="M15 20V10" />
  ...
  { id: "posts", path: "/posts", label: "Посты", icon: (
      <svg className="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
  ...
  { id: "settings", path: "/settings", label: "Настройки", icon: (
      <svg className="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
  ...
  { id: "admin", label: "Админ-панель", icon: (
      <svg className="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
...
function ItemIcon({ children, active }) {
  return (
    <span className={`sidebar__link-icon ${active ? 'ring-2 ring-blue-400/40 bg-blue-50 text-blue-700 dark:bg-white/10 dark:ring-white/20 dark:text-blue-200' : ''}`}>{children}</span>
  );
}
...
export default function Sidebar({ mobileOpen = false, onCloseMobile = () => {} }) {
  ...
  const filteredNav = useMemo(() => {
    ...
    return NAV.filter((it) => {
      if (it.id === 'admin') return allowAll;
      if (it.id === 'analytics') return allowAll || allowNonAdmin || role === 'ANALYTICS';
      if (it.id === 'ai') return allowAll || allowNonAdmin || role === 'NEURAL';
      if (it.id === 'vpn') return allowAll || allowNonAdmin || role === 'VPN';
      if ([ 'docs', 'posts' ].includes(it.id)) return allowAll || allowNonAdmin;
      return true; // home, settings
    });
  }, [role]);
  ...
  const renderLink = (item) => {
      ...
      return (
        <div key={item.id} className="space-y-1">
          <button
            type="button"
            onClick={toggle}
            className={`group relative flex w-full items-center justify-between gap-3 rounded-2xl px-4 py-2 text-sm font-medium transition ${hasActiveChild ? 'bg-gradient-to-r from-blue-500/15 to-indigo-600/15 text-slate-900 ring-1 ring-blue-500/20 dark:from-indigo-500/20 dark:to-blue-600/20 dark:text-white dark:ring-white/10' : 'text-slate-700 hover:bg-black/5 hover:text-slate-900 dark:text-gray-200 dark:hover:bg-white/10 dark:hover:text-white'}`}
          >
            <span className="flex items-center gap-3">
              {groupRootPath ? (
                <NavLink
                  ...
                  className={({ isActive }) => `sidebar__link-icon ${((isActive || hasActiveChild) && isCollapsed) ? 'ring-2 ring-blue-400/40 bg-blue-50 text-blue-700 dark:bg-white/10 dark:ring-white/20 dark:text-blue-200' : ''}`}
                >
                  {item.icon}
                </NavLink>
              ) : (
          ...
          <AnimatePresence initial={false}>
            {isOpen && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.18 }} className="ml-2 space-y-1 pl-3">
                {item.children.map((c) => (
                  <NavLink key={c.id} to={c.path} onClick={() => { if (mobileOpen) onCloseMobile(); }} className={({ isActive }) => `flex items-center gap-3 rounded-2xl px-3 py-2 text-xs transition ${isActive ? 'bg-blue-50 text-blue-700 ring-1 ring-blue-500/20 dark:bg-white/10 dark:text-blue-200' : 'text-slate-600 hover:bg-black/5 hover:text-slate-900 dark:text-gray-300 dark:hover:bg-white/10 dark:hover:text-white'}`}>
    ...
    return (
      <NavLink
        ...
        className={({ isActive }) => `group relative flex items-center gap-3 rounded-2xl px-4 py-2 text-sm font-medium transition ${isActive ? 'bg-gradient-to-r from-blue-500/15 to-indigo-600/15 text-slate-900 ring-1 ring-blue-500/20 dark:from-indigo-500/20 dark:to-blue-600/20 dark:text-white dark:ring-white/10' : 'text-slate-700 hover:bg-black/5 hover:text-slate-900 dark:text-gray-200 dark:hover:bg-white/10 dark:hover:text-white'}`}
      >
  ...
  const brand = (
    <div className={`flex items-center justify-between border-b border-white/10 ${isCollapsed ? 'px-2' : 'px-5'} py-4`}>
      <span className="flex items-center gap-3">
        <a href="/" onClick={(e) => { e.preventDefault(); window.history?.pushState({}, '', '/'); window.dispatchEvent(new PopStateEvent('popstate')); }} className="inline-flex items-center gap-3">
    ...
    <>
      ...
      >
        ...
        <div className="sticky bottom-0 border-t border-white/10 px-3 py-3">
          <button type="button" onClick={() => setIsCollapsed((v) => !v)} className="mx-auto flex h-10 w-10 items-center justify-center rounded-full bg-white/60 text-slate-700 ring-1 ring-black/5 transition hover:bg-white/80 dark:bg-slate-800/70 dark:text-gray-200 dark:ring-white/10" aria-label={isCollapsed ? 'Развернуть сайдбар' : 'Свернуть сайдбар'}>
